...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .zi-link { color: #58a6ff; cursor: pointer; text-decoration: underline; }
        .zi-link:hover { color: #79c0ff; }
        .detalii-zi { background: #0d1117; border: 1px solid #30363d; border-radius: 8px;
            padding: 20px; margin-bottom: 25px; display: none; }
        .detalii-zi.active { display: block; }
        .detalii-titlu { color: #58a6ff; font-size: 18px; margin-bottom: 15px;
            display: flex; justify-content: space-between; align-items: center; }
        .btn-close { background: #21262d; border: 1px solid #30363d; color: #8b949e;
            padding: 5px 12px; border-radius: 6px; cursor: pointer; font-size: 13px; }
        .btn-close:hover { background: #30363d; color: #e6edf3; }
    </style>
</head>
<body>
    <div class="container">
                ...
                <script>
                    (function() {
                        ...
                        if (!d.dates.length) { document.getElementById('chart_{{ p.symbol }}').innerHTML =
                            '<p style="color:#8b949e;text-align:center;padding:40px;">Date indisponibile</p>'; return; }
        ...
        <div id="tab-statistici" class="tab-content">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="zi-link" onclick="arataZi('{{ zi }}')">{{ zi }}</span></td>
                        <td>{{ d.trades }}</td>
                        <td>{{ "%.0f"|format(d.wins / d.trades * 100 if d.trades else 0) }}%</td>
                        <td class="{{ 'green' if d.profit >= 0 else 'red' }}">${{ "%.2f"|format(d.profit) }}</td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>

            <!-- Detalii zi (apare la click pe data) -->
            <div id="detalii-zi" class="detalii-zi">
                <div class="detalii-titlu">
                    <span>📋 Tranzactii din <span id="detalii-zi-data"></span></span>
                    <button class="btn-close" onclick="inchideDetalii()">✕ Inchide</button>
                </div>
                <table id="detalii-zi-tabel">
                    <thead><tr><th>Ora</th><th>Simbol</th><th>Cantitate</th><th>Intrare</th><th>Iesire</th><th>Profit</th><th>%</th><th>Motiv</th><th>Rezultat</th></tr></thead>
                    <tbody id="detalii-zi-body"></tbody>
                </table>
            </div>
    ...
    <script>
        ...
        // Datele tuturor tranzactiilor din CSV-uri
        var toateTrades = {{ toate_trades_json|safe }};

        function arataZi(zi) {
            var tradeZi = toateTrades.filter(function(t) { return t._zi === zi; });
            var body = document.getElementById('detalii-zi-body');
            body.innerHTML = '';

            tradeZi.forEach(function(t) {
                var ora = (t.data_iesire || '').substring(11, 19);
                var profitClass = parseFloat(t.profit_usd) >= 0 ? 'green' : 'red';
                var badge = t.rezultat === 'WIN'
                ...
                var row = '<tr>' +
                    ...
                    '<td><strong>' + t.simbol + '</strong></td>' +
                    '<td>' + t.cantitate + '</td>' +
                    '<td>$' + parseFloat(t.pret_intrare).toFixed(2) + '</td>' +
                    '<td>$' + parseFloat(t.pret_iesire).toFixed(2) + '</td>' +
                    '<td class="' + profitClass + '">$' + parseFloat(t.profit_usd).toFixed(2) + '</td>' +
                    '<td class="' + profitClass + '">' + parseFloat(t.profit_pct).toFixed(2) + '%</td>' +
                    '<td style="font-size:12px;color:#8b949e;">' + (t.motiv_exit || '-') + '</td>' +
                    '<td>' + badge + '</td>' +
                    '</tr>';
                body.innerHTML += row;
            });

            document.getElementById('detalii-zi-data').textContent = zi;
            var el = document.getElementById('detalii-zi');
            el.classList.add('active');
            el.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }

        function inchideDetalii() {
            document.getElementById('detalii-zi').classList.remove('active');
        }
    </script>
</body>
</html>
...
def dashboard():
    ...
    # Pregateste datele de trades pentru JavaScript (link-urile pe zile)
    toate_trades_json = json.dumps(st.get("toate_trades", []) if st else [])
    ...
    return render_template_string(
        ...
        pozitii=pozitii, watchlist=watchlist, st=st,
        toate_trades_json=toate_trades_json,
