...
def analizeaza_semnal(simbol):
    try:
        ...
        crossover_bearish = ema20_prev > ema50_prev and ema20 < ema50
        rsi_ok_short = 30 < rsi < RSI_OVERBOUGHT
        macd_bearish = macd < signal

        if trend_bearish and rsi_ok_short and macd_bearish and volum_ok:
            semnal = "short"
            motiv = (f"EMA20({ema20:.2f})<EMA50({ema50:.2f}) | "
                     f"RSI={rsi:.1f} | MACD bearish | Vol={volum_ratio:.1f}x")
...
def verifica_exit(simbol, pret_intrare, directie, pret_max):
    try:
        ...
        if directie == "long":
            ...
            if ema20 > ema50:
                return True, f"EMA CROSSOVER BULLISH ({variatie:.2%})", pret_max
            if macd > signal and histogram > 0 and variatie < 0:
                return True, f"MACD BULLISH ({variatie:.2%})", pret_max
            if rsi < 25:
                return True, f"RSI OVERSOLD ({rsi:.1f})", pret_max
